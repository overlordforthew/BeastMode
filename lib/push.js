...
const { MIN_DAILY_SESSIONS, syncUserProgressDay } = require("./progress");
...
function normalizeIntervalMinutes(value) {
  ...
  return Math.min(Math.max(Math.round(interval), 1), 720);
}
...
function buildScheduledPushPayload(row, now = new Date()) {
  ...
  const qualified = sessionsFinished >= MIN_DAILY_SESSIONS || meditationsFinished >= 1;
  const remainingWorkouts = Math.max(0, MIN_DAILY_SESSIONS - sessionsFinished);
  const timezone = normalizeTimezone(row.timezone);
  ...
  } else if (!qualified && remainingWorkouts > 0 && meditationsFinished === 0) {
    body = `${remainingWorkouts} more reset${remainingWorkouts === 1 ? "" : "s"} to secure today's streak.`;
  } else if (qualified) {
    body = todayPoints > 0
